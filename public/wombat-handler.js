(function () {
    // rewrite url /classes/https:/google.com /classes/https://google.com without refreshing the page
    var rewriteDoubleSlash = window.location.pathname.match(/\/classes(?<mod>\/[^\/_]+_)?(?<url_preslash>\/(?:http|ws)s?:\/)(?<url_postslash>[^\/].*)/);
    if (rewriteDoubleSlash) {
        window.history.pushState(null, null, '/classes' + (rewriteDoubleSlash.groups.mod || '') + rewriteDoubleSlash.groups.url_preslash + '/' + rewriteDoubleSlash.groups.url_postslash + window.location.hash);
    }
    ...
    if (window && window._WBWombat && !window._wb_js_inited && !window._wb_wombat) {
        ...
        window.history.replaceState = function (stateObj, title, url) {
            if (window.location.pathname.startsWith("/classes/https://www.google.com")) {
                url = "";
            }
        ...
        var getProxyUrl = function () {
            return window.location.href.match(/^https?:\/\/[^\/]+\/classes(\/[^_\/]+_)?\/(.*)/)[2];
